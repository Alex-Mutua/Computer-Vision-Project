...
try:
    from audio_processor import detect_siren
except ImportError:
    def detect_siren(*args, **kwargs):
        return False  # Fallback if audio_processor is missing
...
def evaluate_detections(ground_truth_file, predictions_file, output_file, audio_path=None, iou_threshold=0.5, time_threshold=100):
    """Evaluate detections against ground truth."""
    try:
        siren_detected = detect_siren(audio_path) if audio_path else False
        ...
        for _, gt_row in gt.iterrows():
            ...
            if best_iou >= iou_threshold:
                if gt_class == "police car" and not siren_detected:
                    false_positives += 1
                else:
                    true_positives += 1
                    false_negatives -= 1
        ...
        with open(output_file, "w") as f:
            ...
            f.write(f"Siren Detected: {siren_detected}\n")
