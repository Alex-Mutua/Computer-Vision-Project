    ...
    with col1:
        ...
        if uploaded_video is not None:
            ...
            if st.button("🚀 Process Video", type="primary"):
                processing_caption = "Video is processing, and a new window will open for live detection..." if enable_live_window else f"Processing video with YOLOv11 (confidence: {confidence:.2f}, target: {target_object})..."
                with st.spinner(processing_caption):
                    try:
                        os.makedirs(PREDICTIONS_DIR, exist_ok=True)
                        os.makedirs(OUTPUT_DIR, exist_ok=True)
                        detector_position_log_path = os.path.join(PREDICTIONS_DIR, "position_log.csv")
                        detector_processed_video_path = os.path.join(PREDICTIONS_DIR, "processed_video.mp4")
                        ...
                        if os.path.exists(detector_position_log_path):
                            shutil.copy(detector_position_log_path, position_log_path)
                            if is_valid_csv(position_log_path):
                                df = pd.read_csv(position_log_path)
                                if df.empty or len(df) <= 1:
                                    st.error("No detections in position_log.csv.")
                                else:
                                ...
                                st.warning(f"No valid detections in {position_log_path}. Ensure video contains detectable objects, lower confidence, or check console logs.")
                        else:
                        ...
                        if is_valid_csv(position_log_path):
                            try:
                                df = pd.read_csv(position_log_path)
                                success = generate_heatmap(output_dir=OUTPUT_DIR, target_class=target_object if target_object != "all" else None)
                                if success:
                                    if target_object == "all":
                                        ...
                                        heatmap_path = os.path.join(OUTPUT_DIR, f"heatmap_{target_object.replace(' ', '_')}.png")
                                        if os.path.exists(heatmap_path):
